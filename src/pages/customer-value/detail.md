...
```js
// Map of selected restaurant location
Plot.plot({
  width: 350,
  height: 250,
  projection: {
    ...
    domain: {
      type: "Point",
      coordinates: selectedRestaurant.geometry.coordinates
    },
    inset: 20
  },
  marks: [
    Plot.geo(selectedRestaurant, {
      geometry: d => d.geometry,
      fill: props.value_gap_to_market >= 0 ? "#22c55e" : "#ef4444",
      stroke: "#333",
      strokeWidth: 2,
      r: 10
    }),
    Plot.geo(restaurantGeo.features.filter(f => f.properties.placeId !== selectedPlaceId).slice(0, 50), {
