import mapboxgl, { ensureMapboxAccessToken, createMapboxMap, waitForContainerSize } from "./mapbox-utils.js";
import { coerceGeoJSON, maybeReproject3857To4326 } from "../../lib/geo-utils.js";
import { formatInteger, formatPercent, clampPercent } from "../../lib/formatters.js";
import { MAPBOX_STYLE, COLOR_SCHEMES } from "../../config.js";
...
const DEFAULT_STYLE = MAPBOX_STYLE;
...
export function consumerCentricityMapMapbox({
	...
	// Line overlays with categorical styling (e.g., congestion levels)
	lineOverlays = [], // [{ data, name, property, styleMap, line: { widthDefault, opacity, cap, join } }]
	size,
	layerStyles = {},
	mapboxToken,
	...
	const activateFirstLayer = () => {
		// If there's only one layer, activate it by default
		if (allOverlayNames.length === 1) {
			const name = allOverlayNames[0];
	...
	const addToggleControl = () => {
		...
		const isSingleLayer = allOverlayNames.length === 1;
		for (const name of allOverlayNames) {
			...
			cb.type = "checkbox"; 
			cb.checked = isSingleLayer && visibleNames.has(name); 
	...
	const buildChoroplethWhiteVsTotal = ({ geo, name, property }) => {
		try {
			...
			const red = COLOR_SCHEMES.redBlue.low;
			const blue = COLOR_SCHEMES.redBlue.high;
	...
	const buildChoroplethNumericRedBlue = ({ geo, name, property }) => {
		try {
			const sourceId = `ch-src-${name}`;
			const fillId = `ch-fill-${name}`;
			const lineId = `ch-line-${name}`;
			...
			const dataMax = values.length ? Math.max(...values) : 1;
			const rangeOverride = layerStyles?.[name]?.choropleth?.range;
			const minRange = Array.isArray(rangeOverride) && rangeOverride.length === 2 ? Number(rangeOverride[0]) : dataMin;
			const maxRange = Array.isArray(rangeOverride) && rangeOverride.length === 2 ? Number(rangeOverride[1]) : dataMax;
			const denom = (maxRange - minRange) === 0 ? 1 : (maxRange - minRange);
			const tExpr = ["max", 0, ["min", 1, ["/", ["-", ["to-number", ["get", property]], minRange], denom]]];
			const darkRed = COLOR_SCHEMES.darkRedBlue.low;
			const darkBlue = COLOR_SCHEMES.darkRedBlue.high;
			addLayerOnce({ id: fillId, type: "fill", source: sourceId, paint: { "fill-color": ["interpolate", ["linear"], tExpr, 0, darkRed, 1, darkBlue], "fill-opacity": ["interpolate", ["linear"], tExpr, 0, 0.25, 1, 0.8] } });
			addLayerOnce({ id: lineId, type: "line", source: sourceId, paint: { "line-color": layerStyles?.[name]?.choropleth?.borderColor || "#1f2937", "line-width": layerStyles?.[name]?.choropleth?.borderWidth || 0.6, "line-opacity": 0.7 } });
			const legend = makeLegendContainer();
			legend.style.display = "none";
			const steps = Math.max(3, Math.min(9, layerStyles?.[name]?.choropleth?.steps || 6));
			const stepSize = (maxRange - minRange) / steps;
			...
			for (let i = 0; i < steps; i++) {
				...
				const mid = (a + b) / 2;
				const tMid = Math.max(0, Math.min(1, (mid - minRange) / denom));
				const color = tMid <= 0 ? darkRed : (tMid >= 1 ? darkBlue : undefined);
				const label = `${a.toFixed(0)} – ${b.toFixed(0)}`;
				const opacity = (0.25 + 0.55 * tMid).toFixed(2);
				if (color) {
					rows.push(`<div style="display:flex;align-items:center;margin:2px 0;">
						<span style="display:inline-block;width:14px;height:14px;background:${color};opacity:${opacity};border:1px solid #9ca3af;margin-right:6px;"></span>
						<span>${label}</span>
					...
					rows.push(`<div style="display:flex;align-items:center;margin:2px 0;">
						<span style="display:inline-block;width:14px;height:14px;background:linear-gradient(90deg, ${darkRed}, ${darkBlue});opacity:${opacity};border:1px solid #9ca3af;margin-right:6px;"></span>
						<span>${label}</span>
					</div>`);
				}
			}
			legend.innerHTML = `<div style="font-weight:600;margin-bottom:4px;">${name}</div>` + rows.join("");
			container.appendChild(legend);
			// Popup showing the numeric value
			let numPopup = null;
			try {
				map.on("mouseenter", fillId, () => { container.style.cursor = "pointer"; });
				map.on("mouseleave", fillId, () => { container.style.cursor = ""; });
				map.on("click", fillId, (e) => {
					const feat = (e?.features && e.features[0]) || null;
					const props = feat?.properties || {};
					const val = Number(props[property]);
					const html = `
						<div style="font: 13px/1.3 system-ui, -apple-system, Segoe UI, Roboto, sans-serif;">
							<div style="font-weight:600;margin-bottom:4px;">${name}</div>
							<div>No. de restaurantes: <strong>${formatInteger(val)}</strong></div>
						</div>`;
					try { if (numPopup) { numPopup.remove(); } } catch { /* ignore */ }
					numPopup = new mapboxgl.Popup({ closeButton: true, closeOnMove: true }).setLngLat(e.lngLat).setHTML(html).addTo(map);
				});
			} catch { /* ignore */ }
			return { name, sources: [sourceId], layers: [fillId, lineId], legendEl: legend };
		} catch (e) { derror("choropleth numeric red-blue failed", e); }
		return null;
	};
	...
	const buildChoroplethDriveThru = ({ geo, name, property }) => {
		try {
			...
			const colorHigh = COLOR_SCHEMES.redGradient.high;
	...
	const buildCategoricalPointsOverlay = ({ name, geo, property = "categoryName" }) => {
		try {
			...
			const palette = layerStyles?.[name]?.palette || QUALITATIVE_PALETTE;
	...
	const buildLineOverlay = ({ data, name, property, styleMap = {}, line = {} }) => {
		try {
			...
			try {
				for (const f of (geo.features || [])) {
					const val = String(f?.properties?.[property] ?? "");
					if (val) categories.add(val);
				}
			} catch { /* ignore */ }
			
			// Build color expression
			const matchInput = ["to-string", ["get", property]];
			const colorExpr = ["match", matchInput];
			const sortedCats = Array.from(categories).sort();
			for (const cat of sortedCats) {
				const color = styleMap[cat] || "#6b7280";
			...
			addLayerOnce({
				...
				paint: {
					...
					"line-width": line.widthDefault ?? 2,
			...
			const legend = makeLegendContainer();
			legend.style.display = "none";
			const legendEntries = sortedCats.map((cat) => {
				const color = styleMap[cat] || "#6b7280";
				return `<div style="display:flex;align-items:center;margin:2px 0;">
					<span style="display:inline-block;width:14px;height:3px;background:${color};margin-right:6px;"></span>
					<span>${cat}</span>
			...
			legend.innerHTML = `<div style="font-weight:600;margin-bottom:4px;">${name}</div>` + legendEntries;
			container.appendChild(legend);
	...
	const getChoroplethStrategy = (name, property) => {
		...
		if (property === "CLUSTER_SIZE") return buildChoroplethNumericRedBlue;
